...
function jiraTaskRowHtml(task,selected){
...
function renderJiraTasksWithFields(){
  ...
  root.querySelectorAll('[data-jira-field]').forEach(input=>input.addEventListener('change',()=>{const next=[...root.querySelectorAll('[data-jira-field]:checked')].map(item=>item.dataset.jiraField);if(!next.length){input.checked=true;toast('Cần giữ lại ít nhất một field');return}localStorage.setItem(jiraFieldStorageKey,JSON.stringify(next));renderJiraTasks();requestAnimationFrame(()=>{const picker=root.querySelector('.jira-field-picker');if(picker)picker.open=true})}));
  root.querySelector('.jira-field-reset')?.addEventListener('click',()=>{localStorage.setItem(jiraFieldStorageKey,JSON.stringify(jiraFieldDefaults));renderJiraTasks()});
}
